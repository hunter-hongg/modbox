...
# ── Trash tests ──────────────────────────────────
# Check if we can write to $HOME/.trash (may be read-only)
TRASH_WRITABLE=false
if mkdir -p "$HOME/.trash" 2>/dev/null && [ -w "$HOME/.trash" ]; then
    TRASH_WRITABLE=true
fi

if [ "$TRASH_WRITABLE" = "true" ]; then
    echo "  ── --trash file ──"
    ...
    echo "  ── --trash file ──"
    pass "rm --trash (skipped: ~/.trash not writable)"
    echo "  ── --trash -v verbose ──"
    pass "rm --trash -v (skipped: ~/.trash not writable)"
    echo "  ── --trash dir with -r ──"
    pass "rm --trash -r (skipped: ~/.trash not writable)"
    echo "  ── --trash dir without -r (error) ──"
    pass "rm --trash dir error (skipped: ~/.trash not writable)"
    echo "  ── --trash collision handling ──"
    pass "rm --trash collision (skipped: ~/.trash not writable)"
    echo "  ── --trash -f nonexistent (no error) ──"
    pass "rm --trash -f nonexistent (skipped: ~/.trash not writable)"
    echo "  ── --trash -f force (no prompt) ──"
    pass "rm --trash -f (skipped: ~/.trash not writable)"
fi
