...
/**
 * move_file - Move/rename a regular file from src to dst.
 * Tries rename() first (fast path). Falls back to copy+unlink for
 * cross-filesystem moves.
 *
 * Returns: 0 on success, -1 on error
 */
static int move_file(const char* src, const char* dst) {
    /* Quick check: rename() handles same-filesystem moves */
    if (rename(src, dst) == 0) {
        return 0;
    }
    ...
    if (errno == EXDEV) {
        ...
        if (unlink(src) != 0) {
            // NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
            (void)fprintf(stderr, "mv: %s: Cannot remove source after copy\n", src);
            return -1;
        }
        return 0;
    }
    ...
    /* Some other error */
    // NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
    (void)fprintf(stderr, "mv: cannot move '%s' to '%s'\n", src, dst);
    return -1;
}
...
static int remove_recursive(const char* path) {
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            // NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
            (void)snprintf(child_path, sizeof(child_path), "%s/%s", path, entry->d_name);
...
/**
 * copy_recursive_for_mv - Recursively copy src to dst (for cross-filesystem move).
 *
 * Returns: 0 on success, -1 on error
 */
