...
typedef struct {
    gchar *path;          /* relative path from start point */
    guint64 size_bytes;   /* raw size in bytes (st_blocks*512 or st_size) */
    guint64 agg_size;     /* aggregated size including descendants */
...
static GPtrArray *du_entries;       /* all DuEntry* collected */
...
static int du_callback(const char *fpath, const struct stat *sb,
    ...
    DuEntry *e = g_malloc0(sizeof(DuEntry));
    ...
    if (du_glob_opts->apparent_size) {
        e->size_bytes = (guint64)sb->st_size;
    } else {
        e->size_bytes = (guint64)sb->st_blocks * 512ULL;
    }

    e->agg_size = e->size_bytes;
    g_ptr_array_add(du_entries, e);

    return 0;
}

/* Compare by path for printing */
static gint compare_path(gconstpointer a, gconstpointer b,
                           gpointer user_data) {
    (void)user_data;
    const DuEntry *ea = *(const DuEntry * const *)a;
    const DuEntry *eb = *(const DuEntry * const *)b;
...
static void aggregate_sizes(void) {
    if (du_entries->len == 0) return;

    /* Hash: path → DuEntry* for O(1) parent lookup */
    GHashTable *map = g_hash_table_new(g_str_hash, g_str_equal);
    for (unsigned int i = 0; i < du_entries->len; i++) {
        DuEntry *e = (DuEntry *)g_ptr_array_index(du_entries, i);
    ...
    for (unsigned int i = 0; i < du_entries->len; i++) {
        ...
        *slash = '\0';
        DuEntry *parent = (DuEntry *)g_hash_table_lookup(map, buf);
        if (parent) {
    ...
    g_ptr_array_sort_with_data(du_entries, compare_path, NULL);
}
...
static double scale_size(guint64 bytes, int si, int *suffix_idx) {
...
static char *format_size(guint64 bytes, const DuOptions *opts, char *buf, size_t buf_size) {
    if (opts->bytes) {
        // NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
...
void du_command(gint argc, gchar **argv) {
    ...
    if (exclude_opt->count > 0) {
        opts.exclude = g_malloc((size_t)exclude_opt->count * sizeof(char *));
    ...
    if (threshold_opt->count > 0) {
        ...
        opts.threshold = (guint64)strtoull(threshold_opt->sval[0], &endp, 10);
        if (endp && *endp) {
            switch (*endp) {
    ...
    /* Hash set tracks printed paths; table owns g_strdup'd copies */
    GHashTable *printed = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);

    for (int p = 0; p < path_count; p++) {
        /* Fresh walk for each path */
        du_entries = g_ptr_array_new();
        du_had_error = 0;
        du_scan_count = 0;
        ...
        for (unsigned int i = 0; i < du_entries->len; i++) {
            ...
            if (g_hash_table_contains(printed, e->path)) continue;
            g_hash_table_insert(printed, g_strdup(e->path), GINT_TO_POINTER(1));
        ...
        g_ptr_array_free(du_entries, FALSE);
    }
