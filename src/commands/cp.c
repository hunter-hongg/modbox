...
void cp_command(gint argc, gchar** argv) {
    struct arg_lit* recursive_opt = arg_lit0("r", "recursive", "copy directories recursively");
    /* Collect all remaining positional arguments in one file arg group,
     * then split them into sources and destination manually. */
    struct arg_file* files_arg = arg_filen(NULL, NULL, "SOURCE... DEST", 2, MAX_FILES, "source(s) followed by destination");
    struct arg_end* end = arg_end(20);

    void* argtable[] = { recursive_opt, files_arg, end };
    ...
    if (!is_recursive) {
        ...
        if (num_srcs != 1) {
            // NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
            (void)fprintf(stderr, "cp: expected one source file (use -r for recursive)\n");
            arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
            return;
        }
