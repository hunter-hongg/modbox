...
static std::string compute_sha1(const std::string& filepath, bool is_stdin) {
    SHA_CTX sha1;
    SHA1_Init(&sha1);
    ...
    if (is_stdin) {
        ...
        while (fgets(buffer, sizeof(buffer), stdin)) {
            SHA1_Update(&sha1, buffer, strlen(buffer));
        }
    } else {
...
static bool check_checksums(const std::string& checksum_file, const Sha1SumOptions& opts) {
    std::ifstream file(checksum_file);
    if (!file) {
        fprintf(stderr, "sha1sum: %s: No such file or directory\n", checksum_file.c_str());
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (space_pos == std::string::npos || space_pos != 40) {
            if (opts.warn || opts.strict) {
                fprintf(stderr, "sha1sum: %s: improperly formatted checksum line\n", 
                        checksum_file.c_str());
            }
            if (opts.strict) {
                format_error_count++;
        ...
        char mode_char = line[41]; // Should be ' ' or '*'
        ...
        std::string actual_hash = compute_sha1(filename, false);
    ...
    if (failed_count > 0 || missing_count > 0 || format_error_count > 0) {
        if (failed_count > 0) {
            fprintf(stderr, "sha1sum: WARNING: %d computed checksum did NOT match\n", failed_count);
        }
        if (missing_count > 0) {
            fprintf(stderr, "sha1sum: WARNING: %d listed file could not be read\n", missing_count);
        }
        if (format_error_count > 0) {
            fprintf(stderr, "sha1sum: WARNING: %d line is improperly formatted\n", format_error_count);
        }
        return false;
    }
...
void sha1sum_command(int argc, char** argv) {
    ...
    if (opts.check) {
        if (files_arg->count == 0) {
            fprintf(stderr, "sha1sum: no files specified for check mode\n");
            arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
            return;
        }
    ...
    for (const auto& file : files_to_hash) {
        bool is_stdin = (file == "-");
        std::string hash = compute_sha1(is_stdin ? "" : file, is_stdin);
        ...
        if (opts.tag) {
            printf("SHA1 (%s) = %s", filename.c_str(), hash.c_str());
        } else {
