...
static gint64 head_lines(FILE *fp, gint64 count, int delim, FILE *out) {
    gint64 printed = 0;
    int c;
...
static gint64 head_lines_file(const char *fname, FILE *fp, gint64 count,
...
static gint64 head_bytes(FILE *fp, gint64 count, FILE *out) {
    gint64 printed = 0;
    int c;
...
static gint64 head_bytes_file(const char *fname, FILE *fp, gint64 count,
...
static gint64 parse_count(const char *s, int *is_relative) {
    *is_relative = 0;
    ...
    // NOLINTNEXTLINE(cert-err34-c)
    return (gint64)strtoll(s, NULL, 10);
}
