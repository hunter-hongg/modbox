...
static int lines_equal(const gchar* a, const gchar* b, const UniqOptions* opts) {
    const gchar* pa = a;
    const gchar* pb = b;
    ...
    if (opts->skip_fields > 0) {
        ...
        for (f = 0; f < opts->skip_fields; f++) {
            /* Skip whitespace */
            while (*pa && (guchar)*pa <= ' ') { pa++; }
            while (*pb && (guchar)*pb <= ' ') { pb++; }
            /* Skip non-whitespace */
            while (*pa && (guchar)*pa > ' ') { pa++; }
            while (*pb && (guchar)*pb > ' ') { pb++; }
        }
    ...
    if (opts->ignore_case) {
        ...
        while (*pa && *pb && n < max_cmp) {
            if (g_ascii_tolower(*pa) != g_ascii_tolower(*pb)) {
                return 1;
            }
            pa++;
            pb++;
...
static void output_group(FILE* out_fp, const UniqOptions* opts, const gchar* line, int count) {
...
static void uniq_file(FILE* fp, const UniqOptions* opts, FILE* out_fp) {
    gchar line_buf[UNIQ_MAX_LINE];
    gchar prev_buf[UNIQ_MAX_LINE];
    int has_prev = 0;
    int count = 1;
