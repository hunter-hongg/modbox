...
static std::string extract_key(const char* line, const ParsedKey* key, char sep) {
    const char* start;
    const char* end;
    ...
    return std::string(effective_start, end - effective_start);
}

/* ── Number parsing helpers ─────────────────────────────────────────────── */

static double parse_numeric(const char* s, int* ok) {
    char* end = nullptr;
    /* Skip leading blanks */
    while (*s == ' ' || *s == '\t') {
        s++;
...
static int compare_key_strings(const std::string& a, const std::string& b,
                                const ParsedKey* key) {
...
static int compare_content(const SortLine& la,
                             const SortLine& lb,
...
static int compare_sort_lines_direct(const SortLine& la,
                                       const SortLine& lb,
                                       const SortOptions* opts) {
...
static ParsedKey parse_one_key(const char* spec) {
    ParsedKey key;
    const char* p = spec;
...
static void add_key_from_spec(const char* spec, std::vector<ParsedKey>& keys) {
    keys.push_back(parse_one_key(spec));
}

/* ── I/O: reading lines ─────────────────────────────────────────────────── */

static std::vector<SortLine> read_lines(int file_count, const char** filenames) {
...
static int check_sorted(const std::vector<SortLine>& lines, const SortOptions* opts,
                         int file_count, const char** filenames) {
    for (std::size_t i = 1; i < lines.size(); i++) {
        const SortLine& prev = lines[i - 1];
        ...
        if (cmp > 0) {
            ...
            (void)fprintf(stderr, "sort: %s: line %d: disorder: %s\n",
                          fname, line_num + 1, curr.line.c_str());
            return 1;
        }
...
void sort_command(int argc, char** argv) {
    ...
    std::sort(lines.begin(), lines.end(),
              [&opts](const SortLine& a, const SortLine& b) {
                  return compare_sort_lines_direct(a, b, &opts) < 0;
              });
