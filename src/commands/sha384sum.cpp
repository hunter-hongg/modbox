...
static std::string compute_sha384(const std::string& filepath, bool is_stdin) {
    SHA512_CTX sha384;
    SHA384_Init(&sha384);
    ...
    if (is_stdin) {
        ...
        while (fgets(buffer, sizeof(buffer), stdin)) {
            SHA384_Update(&sha384, buffer, strlen(buffer));
        }
    } else {
        ...
        while (file.read(buffer, sizeof(buffer))) {
            SHA384_Update(&sha384, buffer, file.gcount());
        }
        if (file.gcount() > 0) {
            SHA384_Update(&sha384, buffer, file.gcount());
        }
    ...
    char hex_hash[SHA384_DIGEST_LENGTH * 2 + 1];
    for (int i = 0; i < SHA384_DIGEST_LENGTH; i++) {
        sprintf(hex_hash + (i * 2), "%02x", hash[i]);
    }
    hex_hash[SHA384_DIGEST_LENGTH * 2] = '\0';

    return std::string(hex_hash);
}

static bool check_checksums(const std::string& checksum_file, const Sha384SumOptions& opts) {
    std::ifstream file(checksum_file);
    if (!file) {
        fprintf(stderr, "sha384sum: %s: No such file or directory\n", checksum_file.c_str());
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (space_pos == std::string::npos || space_pos != 96) {
            if (opts.warn || opts.strict) {
                fprintf(stderr, "sha384sum: %s: improperly formatted checksum line\n",
                        checksum_file.c_str());
            }
            if (opts.strict) {
                format_error_count++;
        ...
        std::string actual_hash = compute_sha384(filename, false);
    ...
    if (failed_count > 0 || missing_count > 0 || format_error_count > 0) {
        if (failed_count > 0) {
            fprintf(stderr, "sha384sum: WARNING: %d computed checksum did NOT match\n", failed_count);
        }
        if (missing_count > 0) {
            fprintf(stderr, "sha384sum: WARNING: %d listed file could not be read\n", missing_count);
        }
        if (format_error_count > 0) {
            fprintf(stderr, "sha384sum: WARNING: %d line is improperly formatted\n", format_error_count);
        }
        return false;
    }
...
void sha384sum_command(int argc, char** argv) {
    ...
    if (help_opt->count > 0) {
        ...
        printf("Print or check SHA384 (384-bit) checksums.\n");
        printf("\n");
    ...
    if (opts.check) {
        if (files_arg->count == 0) {
            fprintf(stderr, "sha384sum: no files specified for check mode\n");
            arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
            return;
        }
    ...
    for (const auto& file : files_to_hash) {
        bool is_stdin = (file == "-");
        std::string hash = compute_sha384(is_stdin ? "" : file, is_stdin);
        ...
        if (opts.tag) {
            printf("SHA384 (%s) = %s", filename.c_str(), hash.c_str());
        } else {
