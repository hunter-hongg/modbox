...
static int check_section_delimiter(const gchar* line, const gchar delim[2]) {
    if (strlen(line) < 2) {
        return 0;
    }
...
static int should_number(const gchar* line, const gchar* style, int is_empty) {
    ...
    /* pBRE format */
    if (style[0] == 'p') {
        const gchar* bre = style + 1;
        if (*bre == '\0') {
            return 1; /* empty BRE matches all */
        }
        /* Use g_regex_match_simple with BRE-like pattern.
         * Since g_regex uses Perl-compatible syntax, we approximate BRE.
         * For simple patterns this works; for full BRE support a real
         * BRE matcher would be needed. */
        GRegex* regex = g_regex_new(bre, G_REGEX_RAW, 0, NULL);
        if (regex == NULL) {
            return 1; /* fallback: number all */
        }
        int match = g_regex_match(regex, line, 0, NULL);
        g_regex_unref(regex);
        return match;
    }
    return 1; /* fallback */
...
static void format_number(int num, const gchar* fmt, int width, gchar* buf, size_t buf_size) {
...
void nl_command(gint argc, gchar** argv) {
    ...
    /* Set defaults */
    opts.body_numbering = g_strdup(body_numbering_opt->count > 0 ? body_numbering_opt->sval[0] : "a");
    opts.header_numbering = g_strdup(header_numbering_opt->count > 0 ? header_numbering_opt->sval[0] : "t");
    opts.footer_numbering = g_strdup(footer_numbering_opt->count > 0 ? footer_numbering_opt->sval[0] : "t");
    opts.number_format = g_strdup(num_format_opt->count > 0 ? num_format_opt->sval[0] : "rn");
    ...
    if (separator_opt->count > 0) {
        opts.number_separator = g_strdup(separator_opt->sval[0]);
    } else {
        opts.number_separator = g_strdup("\t");
    }

    /* Section delimiter: default is backslash-colon */
    if (section_delim_opt->count > 0) {
        const gchar* d = section_delim_opt->sval[0];
    ...
    if (file_arg->count > 0) {
        ...
        if (strcmp(fname, "-") != 0) {
            ...
            if (fp == NULL) {
                ...
                g_free(opts.body_numbering);
                g_free(opts.header_numbering);
                g_free(opts.footer_numbering);
                g_free(opts.number_format);
                g_free(opts.number_separator);
                arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
                return;
            }
        }
