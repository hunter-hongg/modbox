...
static std::string compute_sha512(const std::string& filepath, bool is_stdin) {
    SHA512_CTX sha512;
    SHA512_Init(&sha512);
    ...
    if (is_stdin) {
        ...
        while (fgets(buffer, sizeof(buffer), stdin)) {
            SHA512_Update(&sha512, buffer, strlen(buffer));
        }
    } else {
...
static bool check_checksums(const std::string& checksum_file, const Sha512SumOptions& opts) {
    std::ifstream file(checksum_file);
    if (!file) {
        fprintf(stderr, "sha512sum: %s: No such file or directory\n", checksum_file.c_str());
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (space_pos == std::string::npos || space_pos != 128) {
            if (opts.warn || opts.strict) {
                fprintf(stderr, "sha512sum: %s: improperly formatted checksum line\n",
                        checksum_file.c_str());
            }
            if (opts.strict) {
                format_error_count++;
        ...
        std::string actual_hash = compute_sha512(filename, false);
    ...
    if (failed_count > 0 || missing_count > 0 || format_error_count > 0) {
        if (failed_count > 0) {
            fprintf(stderr, "sha512sum: WARNING: %d computed checksum did NOT match\n", failed_count);
        }
        if (missing_count > 0) {
            fprintf(stderr, "sha512sum: WARNING: %d listed file could not be read\n", missing_count);
        }
        if (format_error_count > 0) {
            fprintf(stderr, "sha512sum: WARNING: %d line is improperly formatted\n", format_error_count);
        }
        return false;
    }
...
void sha512sum_command(int argc, char** argv) {
    ...
    if (help_opt->count > 0) {
        ...
        printf("Print or check SHA512 (512-bit) checksums.\n");
        printf("\n");
    ...
    if (opts.check) {
        if (files_arg->count == 0) {
            fprintf(stderr, "sha512sum: no files specified for check mode\n");
            arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
            return;
        }
    ...
    for (const auto& file : files_to_hash) {
        bool is_stdin = (file == "-");
        std::string hash = compute_sha512(is_stdin ? "" : file, is_stdin);
        ...
        if (opts.tag) {
            printf("SHA512 (%s) = %s", filename.c_str(), hash.c_str());
        } else {
