...
static constexpr int SHA224_HEX_LEN = 56;

static std::string compute_sha224(const std::string& filepath, bool is_stdin) {
    SHA256_CTX sha224;
    SHA224_Init(&sha224);
    ...
    if (is_stdin) {
        ...
        while (fgets(buffer, sizeof(buffer), stdin)) {
            SHA224_Update(&sha224, buffer, strlen(buffer));
        }
    } else {
        ...
        while (file.read(buffer, sizeof(buffer))) {
            SHA224_Update(&sha224, buffer, file.gcount());
        }
        if (file.gcount() > 0) {
            SHA224_Update(&sha224, buffer, file.gcount());
        }
    ...
    char hex_hash[SHA224_DIGEST_LENGTH * 2 + 1];
    for (int i = 0; i < SHA224_DIGEST_LENGTH; i++) {
        sprintf(hex_hash + (i * 2), "%02x", hash[i]);
    }
    hex_hash[SHA224_DIGEST_LENGTH * 2] = '\0';

    return std::string(hex_hash);
}

static bool check_checksums(const std::string& checksum_file, const Sha224SumOptions& opts) {
    std::ifstream file(checksum_file);
    if (!file) {
        fprintf(stderr, "sha224sum: %s: No such file or directory\n", checksum_file.c_str());
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (space_pos == std::string::npos || space_pos != SHA224_HEX_LEN) {
            if (opts.warn || opts.strict) {
                fprintf(stderr, "sha224sum: %s: improperly formatted checksum line\n",
                        checksum_file.c_str());
            }
            if (opts.strict) {
                format_error_count++;
        ...
        std::string expected_hash = line.substr(0, SHA224_HEX_LEN);
        char mode_char = line[SHA224_HEX_LEN + 1];
        std::string filename = line.substr(SHA224_HEX_LEN + 2);
        ...
        std::string actual_hash = compute_sha224(filename, false);
    ...
    if (failed_count > 0 || missing_count > 0 || format_error_count > 0) {
        if (failed_count > 0) {
            fprintf(stderr, "sha224sum: WARNING: %d computed checksum did NOT match\n", failed_count);
        }
        if (missing_count > 0) {
            fprintf(stderr, "sha224sum: WARNING: %d listed file could not be read\n", missing_count);
        }
        if (format_error_count > 0) {
            fprintf(stderr, "sha224sum: WARNING: %d line is improperly formatted\n", format_error_count);
        }
        return false;
    }
...
void sha224sum_command(int argc, char** argv) {
    ...
    if (opts.check) {
        if (files_arg->count == 0) {
            fprintf(stderr, "sha224sum: no files specified for check mode\n");
            arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
            return;
        }
    ...
    for (const auto& file : files_to_hash) {
        bool is_stdin = (file == "-");
        std::string hash = compute_sha224(is_stdin ? "" : file, is_stdin);
        ...
        if (opts.tag) {
            printf("SHA224 (%s) = %s", filename.c_str(), hash.c_str());
        } else {
