...
static std::string compute_sha256(const std::string& filepath, bool is_stdin) {
    SHA256_CTX sha256;
    SHA256_Init(&sha256);
...
static bool check_checksums(const std::string& checksum_file, const Sha256SumOptions& opts) {
    ...
    while (std::getline(file, line)) {
        ...
        if (space_pos == std::string::npos || space_pos != 64) {
            if (opts.warn || opts.strict) {
                fprintf(stderr, "sha256sum: %s: improperly formatted checksum line\n", 
                        checksum_file.c_str());
            }
            if (opts.strict) {
                format_error_count++;
        ...
        char mode_char = line[65]; // Should be ' ' or '*'
        ...
        std::string actual_hash = compute_sha256(filename, false);
    ...
    if (failed_count > 0 || missing_count > 0 || format_error_count > 0) {
        if (failed_count > 0) {
            fprintf(stderr, "sha256sum: WARNING: %d computed checksum did NOT match\n", failed_count);
        }
        if (missing_count > 0) {
            fprintf(stderr, "sha256sum: WARNING: %d listed file could not be read\n", missing_count);
        }
        if (format_error_count > 0) {
            fprintf(stderr, "sha256sum: WARNING: %d line is improperly formatted\n", format_error_count);
        }
        return false;
    }
...
void sha256sum_command(int argc, char** argv) {
    ...
    if (opts.check) {
        if (files_arg->count == 0) {
            fprintf(stderr, "sha256sum: no files specified for check mode\n");
            arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
            return;
        }
    ...
    for (const auto& file : files_to_hash) {
        bool is_stdin = (file == "-");
        std::string hash = compute_sha256(is_stdin ? "" : file, is_stdin);
        ...
        if (opts.tag) {
            printf("SHA256 (%s) = %s", filename.c_str(), hash.c_str());
        } else {
