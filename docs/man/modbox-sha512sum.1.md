% MODBOX-SHA512SUM(1) modbox | User Commands
% modbox project
% 2026-08-09

# NAME

modbox-sha512sum - compute and check SHA512 (512-bit) checksums

# SYNOPSIS

**modbox sha512sum** [*OPTION*]... [*FILE*]...
...
**--tag**
:   Produce a BSD-style checksum line of the form `SHA512 (file) = hash`.
...
```bash
# Compute the SHA512 checksum of a file
modbox sha512sum file.iso

# Compute checksums of several files at once
modbox sha512sum file1 file2 file3 > checksums.sha512

# Verify checksums recorded earlier
modbox sha512sum -c checksums.sha512

# Checksum standard input from a pipeline
cat file.iso | modbox sha512sum

# Verify strictly, reporting only the final status
modbox sha512sum -c --status --strict checksums.sha512
```
