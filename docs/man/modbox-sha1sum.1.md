% MODBOX-SHA1SUM(1) modbox | User Commands
% modbox project
% 2026-08-09

# NAME

modbox-sha1sum - compute and check SHA1 (160-bit) checksums

# SYNOPSIS

**modbox sha1sum** [*OPTION*]... [*FILE*]...
...
**--tag**
:   Produce a BSD-style checksum line of the form `SHA1 (file) = hash`.
...
modbox sha1sum file.iso

# Compute checksums of several files at once
modbox sha1sum file1 file2 file3 > checksums.sha1

# Verify checksums recorded earlier
modbox sha1sum -c checksums.sha1

# Checksum standard input from a pipeline
cat file.iso | modbox sha1sum

# Verify strictly, reporting only the final status
modbox sha1sum -c --status --strict checksums.sha1
```
